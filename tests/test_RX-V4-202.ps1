...
Write-Host "Top Candidate Name:   $($handoff.top_candidate_name)"
Write-Host "Top Candidate Ref:    $($handoff.top_candidate_ref)"
Write-Host "Top Candidate Score:  $($handoff.top_candidate_score)"
Write-Host "Task Title:           $($handoff.task_title)"

Write-Host ""
if ($handoff.verification_status -eq "possible_match") {
    Write-Host "PASS: $callId returned possible_match" -ForegroundColor Green
} else {
    Write-Host "FAIL: expected possible_match but got $($handoff.verification_status)" -ForegroundColor Red
}