...
def test_dashboard_pages_render_after_import(tmp_path, monkeypatch):
    ...
    assert "View Critical" in index_response.text
    assert import_response.status_code == 303
    assert detail_response.status_code == 200
    assert "RAWMOCK-006-URGENT-REDFLAG" in detail_response.text
    assert "Emergency red-flag case - immediate staff review required" in detail_response.text
...
def test_sidebar_navigation_and_dashboard_requests_split(tmp_path, monkeypatch):
    ...
    assert 'class="sidebar"' in dashboard.text
    ...
    assert "View Critical" in dashboard.text
...
def test_dashboard_and_requests_content_are_separated(tmp_path, monkeypatch):
    ...
    assert "System Health" not in requests.text
...
def test_rawmock_cards_show_friendly_locked_values(tmp_path, monkeypatch):
    ...
    assert 'class="case-pill-row" aria-label="Primary status"' in response.text
...
def test_rawmock_006_emergency_marker(tmp_path, monkeypatch):
    ...
    assert "Emergency red-flag case - immediate staff review required" not in status_section
...
def test_recent_audit_history_renders_without_old_new_values(tmp_path, monkeypatch):
    ...
    assert "Show technical audit details" in detail.text
...
def test_dashboard_renders_compact_case_row_hooks_and_expanded_copy_buttons(tmp_path, monkeypatch):
    ...
    assert "case-pill-row" in response.text
    assert "request-type-col" in response.text
    assert "Patient Details" in response.text
    assert "Summary" in response.text
    assert detail.status_code == 200
    assert "Copy Patient Record Note" in detail.text
...
def test_call_column_does_not_render_workflow_status_pill(tmp_path, monkeypatch):
    ...
    assert response.status_code == 200
    assert "call-cell" in response.text
    assert "Caller ID" in response.text
    call_section = response.text.split('class="request-cell call-cell"')[1].split('class="request-cell patient-details-cell"')[0]
...
def test_red_flag_case_row_remains_visually_marked_and_batch_checkbox_present(tmp_path, monkeypatch):
    ...
    assert response.status_code == 200
    assert 'class="request-row emergency' in response.text
...
def test_secondary_pages_have_dashboard_navigation(tmp_path, monkeypatch):
    ...
    assert "Back to Patient Requests" in detail.text
...
def test_status_column_renders_single_primary_status_only(tmp_path, monkeypatch):
    ...
    assert "Emergency red-flag case - immediate staff review required" not in red_row.split('class="request-cell status-cell"')[1]
...
def test_dashboard_selection_controls_and_filter_bar_render(tmp_path, monkeypatch):
    ...
    assert "Quick filters" in response.text
    ...
    assert "data-bulk-action-button" in response.text
    assert "const selectedCallIds = new Set()" in response.text
...
def test_action_needed_layout_groups_copy_buttons(tmp_path, monkeypatch):
    ...
    assert response.status_code == 200
    assert 'class="copy-action-row"' in response.text
    copy_row = response.text.split('class="copy-action-row"')[1].split("</div>")[0]
    assert "Copy Patient Record Note" in copy_row
