...
- Fix the security vulnerability in n8n.py by removing the /api/n8n/test-intake-batch endpoint due to an HMAC failure, as signed off by Saeed.
- The authentication logic endpoint is currently open.
- Rotate the voice agent secret file within the git history for security purposes, rather than simply updating it.
- Stop tracking changes.
- Access control lists for the local configuration files are set so that authorized users can write to them, and this task was completed on July 20, 2026, by Saeed.
