...
- To improve security, we need to fix the HMAC failure and remove the test intake batch API endpoint.
- The authentication endpoint is currently open.
- We must rotate the voice agent secret key within the Git history for security purposes.
- The tracking status remains untracked.
- Access control lists for the local configuration files were completed by Saeed on July 20, 2026.
