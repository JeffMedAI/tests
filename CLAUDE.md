...
- **The 19:00 brief no longer closes anything.** It reads the marker at `logs\close-state\YYYY-MM-DD-close.txt` and reports. If the marker for a day a close fell due is missing, it puts a day-named close-failure banner at the top of the WhatsApp message (**"FRIDAY'S SESSION CLOSE DID NOT COMPLETE"** — never "today's"; see the bullet below). There is deliberately no silent fallback close — a quiet auto-recovery is how the 11–19 Aug 2026 failure hid for eight days.
...
- **Weekends get no close** — no session log, no HANDOFF refresh, no restore tag. Weekend work is still committed and pushed by the next 07:00 morning brief, so nothing is stranded. The Saturday and Sunday evening briefs say one plain line explaining that no close runs today. **They still report on the last close that actually fell due** (normally Friday's), so a Friday failure is not buried by the weekend and keeps showing until it is fixed. A hand-run close (`session_close.ps1 -Force`) files its marker under the day it ran, and the weekend brief reads that too, so a failed or push-held weekend close is never dropped. Corrected 2026-09-07 — this bullet previously said the weekend brief does not fire the close-failure banner, which was the earlier, wrong design: it made a Friday failure invisible from Saturday morning onward.
- **A close failure keeps reminding Saeed until it is fixed** — Saeed's decision 2026-09-07, asked and answered explicitly. A Friday failure therefore re-appears in the Saturday and Sunday evening briefs. That is deliberate repetition, not a bug: do not "de-duplicate" it. It is safe to repeat only because each reminder names the day it is about (next bullet); an unfixed failure repeating as "TODAY'S" would be the cry-wolf problem again.
- **Every alarm names its day.** The brief reports on the close that last fell due, which is often not today's, so the banners say "FRIDAY'S SESSION CLOSE DID NOT COMPLETE", not "TODAY'S". Do not "simplify" these back to "today" — sending Saeed to check the wrong day's scheduled task is how a banner stops being believed.
- **"I cannot see this project" is its own alarm, and it outranks everything.** If a project's `docs\sessions\` folder is missing or unreadable, the brief prints a **CANNOT SEE ONE OF YOUR PROJECTS** banner above all others. It fires even for a paused project — pausing means "no new work expected", never "the folder may vanish". Before 2026-09-07 a missing folder produced silence and an unreadable one killed the whole brief with no message at all; that is the exact shape of the 11–19 Aug 2026 outage.
- **A pause expires into a question.** After 7 days (`$PausedNagAfterHours` in `combined_brief.ps1`) the quiet paused note starts asking Saeed to confirm the pause is still correct. Saeed's instruction 2026-09-07 — a pause set and forgotten must not become a permanent blind spot.
