...
- **Weekends get no close** — no session log, no HANDOFF refresh, no restore tag. Weekend work is still committed and pushed by the next 07:00 morning brief, so nothing is stranded. The Saturday and Sunday evening briefs therefore say one plain line explaining that, and do **not** fire the close-failure banner: no close was scheduled, so a missing marker is the expected state. A marker that says FAILED is still loud on a weekend (someone ran a close by hand and it broke). Added 2026-09-07 — before this, the most important banner in the system cried wolf 104 times a year.
