...
**Pilot site:** Churchtown Medical Centre, Southport — NOT YET LIVE
...
security | GDPR, NHS, OWASP — VETO authority over all PRs
...
## CURRENT STATUS — 2026-06-08 02:42
...
- WhatsApp alerts: MUTED (flag file at logs/service_control/alerts_muted) — delete to re-enable
...
4. SafeToQueueOverride added to Get-JeffHandoffDisposition — DONE
5. Per-user random salt for password/PIN hashing — DONE
6. Session tokens hashed before DB storage — DONE (Saeed override of Security veto, written 2026-06-08)
...
- Cannot kill without admin Task Manager — harmless since alerts are muted
...
1. **Kill elevated watchdog ghost** — open Task Manager as admin, kill stale powershell.exe watchdog, then delete alerts_muted flag to re-enable alerts
2. **NHS SBS Ariba registration** — DEADLINE 23 June 2026. Register at ariba.com before submission.
...
-----+-----------------------------------------+----------+-------------------------
 1   | Phase 3 sprint                          | Backend  | NEXT — GDPR purge DB,
 ...
 3   | 5 deadletter queue items                | Backend  | No replay tooling — tech debt
...
C:\JeffLocal\CLAUDE.md                          ← Rules (read every session)
C:\JeffLocal\docs\sessions\                     ← Per-session summaries
C:\JeffLocal\docs\reports\                      ← Daily reports (YYYY-MM-DD.md)
C:\JeffLocal\dashboard\app\main.py              ← PRODUCTION Flask app
C:\JeffLocal\dashboard\app\auth.py              ← Auth module
C:\JeffLocal\dashboard\app\db.py                ← Database module
C:\JeffLocal\app\process_queue.ps1              ← PowerShell pipeline
C:\JeffLocal\app\modules\Jeff.Handoff.ps1       ← Handoff disposition logic
C:\JeffLocal\app\build_handoff.ps1              ← Handoff builder
C:\JeffLocal\scripts\daily\send_whatsapp.py     ← WhatsApp alert sender
C:\JeffLocal\scripts\service_control\watchdog.ps1 ← Service watchdog
C:\JeffLocal\logs\service_control\alerts_muted  ← DELETE THIS to re-enable WhatsApp alerts
C:\JeffLocal\config\model_settings.json         ← model: gemma4:e2b, temp: 0.1
...
Branch:  sandbox
Latest:  de35cf8 — fix: add mute flag to send_whatsapp.py
...
Auth:         Session cookies (httponly, samesite=lax) — tokens hashed in DB
...
5. **Security Agent reviews ALL PRs** — even one-line changes. Veto is independent.
...
3. Read docs\sessions\ — yesterday's and today's logs
...
2. Update this file — status, tasks, git state
